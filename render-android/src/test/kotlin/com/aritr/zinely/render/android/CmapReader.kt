...
/**
 * A minimal, dependency-free TrueType/OpenType `cmap` reader that returns the set of Unicode code
 * points a font file **itself** maps to a non-`.notdef` glyph (ADR-028 §4.2). This is the correct
 * tool for the bundled-font coverage guard: `Paint.hasGlyph` walks the **whole system fallback chain**,
 * so it would falsely pass a glyph that the bundled font lacks but a device font supplies — exactly the
 * drift the guard exists to prevent. Parsing the `cmap` directly answers "does *this* TTF cover it?".
 *
 * Handles the two Unicode subtable formats real fonts use: format 4 (BMP segment mapping) and format 12
 * (full UCS-4 groups). The MVP charset ([MvpCharset]) is entirely BMP, so format 4 suffices for Inter,
 * but format 12 is parsed too so the guard stays correct if a future bundled font is UCS-4 only.
 */
internal object CmapReader {

    fun coveredCodePoints(data: ByteArray): Set<Int> {
