...
class SurfaceTraversalOrderTest {
    ...
    // ---------------------------------------------------------------------------------------------
    // Entry point 1 · "Which zine do I want?" — ⚠ CURRENTLY UNCOVERED, and deliberately left that way
    // ---------------------------------------------------------------------------------------------
    //
    // This slot held `shelf traversal order is the design's order and the visual order`, which rendered
    // `HomeScreen` — the V1 shelf. `ZinelyNavHost` had already stopped routing to it, so the suite's
    // coverage of the app's FIRST screen was coverage of a screen no user could reach, while the live
    // `ZineLibraryScreen` had none at all. The test was deleted with the screen.
    //
    // It is NOT replaced here, and the gap is written down rather than quietly opened. A Library case
    // needs `assertLogicalTraversalOrder` (private to this class) against a surface whose spoken labels
    // live as private constants in `feature/library/` — so it is either a real refactor of this suite's
    // helpers or a guessed expectation, and a guessed traversal expectation is worse than none: it
    // passes, and it pins the wrong order. Owner's call which way it lands.
