...
| [docs/planning/ZINELY-1X-IMPLEMENTATION-PLAN.md](docs/planning/ZINELY-1X-IMPLEMENTATION-PLAN.md) | Zinely 1.x plan (2026-09-25) — directions, foundations, sequence, owner decisions, and implementation briefs in [docs/planning/](docs/planning/). **Step 0 (guards) complete** (PR #75, 2026-09-26); every later step still needs its owner rulings and a fresh, reviewed session |
...
| [docs/COMPOSE-V2-ROADMAP.md](docs/COMPOSE-V2-ROADMAP.md) | *What*, in order — phases A–F with their deliverables, acceptance criteria and review gates, plus the **[Phase A completion record](docs/COMPOSE-V2-ROADMAP.md#phase-a-completion-record-2026-07-29)**: what was actually built, what was built differently than planned, and the criterion re-seated to Phase D by the D-016 ruling |
| [config/token-enrolment.txt](config/token-enrolment.txt) | The committed definition of which packages the CI-27 token-discipline gate scans. **Currently empty** — a package joins in the same commit that migrates it off raw literals |
| [docs/design/V2-TOKENS.md](docs/design/V2-TOKENS.md) | The **locked V2 palette** — the corpus the Compose tokens are parity-tested against at run time ([ADR-079](docs/DECISIONS.md#adr-079)) |
