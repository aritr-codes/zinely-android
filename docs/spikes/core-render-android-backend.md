...
- A **coverage test** asserts every MVP-charset character resolves within the bundled family (via `Paint.hasGlyph` / font `cmap`), failing the build if a bundled font regresses coverage.
...
| # | Risk | Sev | Mitigation |
|---|---|---|---|
| R1 | **Text rasterisation under Robolectric** — legacy mode = placeholder glyphs, no real metrics | High | Pin `robolectric.graphicsMode=NATIVE` (real Skia text); escalate to instrumented Roborazzi if NATIVE text is non-deterministic on the runner (§7.1) |
...
| **G5** | Canvas providers — `PdfDocument` + `Bitmap` (export) + `PdfRenderer` rasterise harness (§7.3); raw-Canvas seam for the preview host (Compose host lands in S4) | both providers render a page; PDF rasterises back |
| **G6** | **Roborazzi golden + parity diffs** at **multiple scales** (screen + fractional zoom + 300 DPI) for text/image-FIT/FILL/crop/rotation/z-overlap/placeholder; raster-vs-PDF parity; PDF structural check; all variables pinned on the pinned toolchain (§7.3, §7.5) | **diffs green in CI ⇒ S3 raster+PDF parity COMPLETE** (Compose preview parity owed by S4, §2.4) |
...
|---|---|---|
| Q1 | Where does the **Compose preview host** live — `:render-android` or `feature:editor` (S4)? | **`feature:editor`.** Keep `:render-android` Compose-free; the host is a ~5-line `drawIntoCanvas` bridge over the same `CanvasReplayer`. Smaller deps; S3 parity needs no Compose. **Cost (binding):** S4 *must* add a Compose-host Roborazzi parity test — S3 proves raster+PDF only (§2.4, §7.3, Required-fix C). *Final placement deferred to S4 — non-blocking for S3.* |
...
| C | S3 can't prove **Compose preview** parity while the host is deferred; exit bar must say "raw replayer parity"; S4 owes a Compose-host test | Required | **ACCEPTED** — scope-honest exit bar (§1, §9 G6); S4 obligation recorded (§2.4, Q1) |
