...
| **G6b** | **Text goldens + image-fidelity execution.** (a) Text (wrap/align/bold-italic/overflow-clip) at each scale + the §4.2 **bundled self-covering font map + glyph-coverage test** — needs a committed licence-clear TTF (the single most drift-prone raster). (b) When an emulator/connected-androidTest job exists, **run** `ImageReplayInstrumentedTest` + the PDF parity/`no-image-XObject` checks (currently compile-checked only). | text goldens green + image/PDF instrumented proofs executed ⇒ **S3 raster+PDF parity COMPLETE** (Compose preview parity owed by S4, §2.4) |
