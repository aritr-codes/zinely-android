...
**Still next:** Roborazzi selection-chrome goldens (needs the branch on CI). Deferred debt unchanged (compose-ui `LocalLifecycleOwner`; English labels; reducer rotation-normalize + max-size clamp). Follow-ups: real centring/zoom viewport (MVP is contain-fit, pan = 0); a host interaction-lock for the pathological multi-pointer case; wiring `pageSizePt` from imposition + a real image pipeline + autosave binder at the app/DI layer.
