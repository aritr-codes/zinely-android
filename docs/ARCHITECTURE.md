...
Module split (realised vs planned): **realised** — `:app` (shelf/nav and Android hosts), the pure `:core:model|imposition|data|data-storage|render|editor` modules, `:render-android`, `:feature:editor`, `:core:ui`, and `:data-android`. The latter owns file persistence, the rebuildable Room index ([ADR-042](DECISIONS.md#adr-042)), library backup/restore repository coordination, and the thin SAF transport ([ADR-110](DECISIONS.md#adr-110)); WorkManager asset GC remains deferred. **Planned** module extractions remain `:core:domain` and `:feature:home|export|settings`; current screens stay where their existing consumers and test infrastructure live until another consumer justifies a split ([ADR-043](DECISIONS.md#adr-043)).

**Backup implementation boundary (2026-08-22):** `:core:data` owns [ADR-110](DECISIONS.md#adr-110)'s v2 whole-library contract. `:core:data-storage` owns both streamed directions: hostile input is verified, decoded, closure-checked, and staged privately; backup production validates a files-as-truth snapshot and writes a manifest-first archive while refusing overwrite, tampered sources, limits, and cancellation without leaving incomplete private output. The expanded-content limit is 8 GiB, while a separate 8 GiB + 64 MiB encoded-archive envelope safely accommodates bounded ZIP64 framing and no-compression DEFLATE overhead. `:data-android` coordinates both directions under the existing library-writer lease. Its thin `LibrarySafTransport` copies non-seekable provider input to bounded private cache before trusted restore and copies a complete private backup to a user-selected destination. Provider failures and cancellation clean private transfer residue and never write live project paths. The temporary debug picker harness used for real-device verification is not part of the implementation; the frozen production user surface and final product copy remain unimplemented, and provider delivery is necessarily best-effort rather than app-private atomic. Room remains a rebuildable index rather than backup authority.
...
### 4.1 Compatibility guards and the `.zine` v2 format note

The persisted formats are guarded by frozen artefacts, not only by round-trip tests (1.x implementation plan, step 0, foundations F1a and F3):

- **Document shape** — `DocumentSchemaShapeTest` (`:core:model`) walks the `ZineDocument` serializer descriptors and compares the rendered shape (every type, field name, field type and has-a-default flag reachable from the root) with the recorded snapshot `schema-shape-v<N>.txt` for the current `CURRENT_SCHEMA_VERSION`. The test fails when the two diverge while the version is unchanged — for example when a field is added, as ADR-106's `copier` was — and when no snapshot exists for the current version. Why an unrecorded field matters: `ignoreUnknownKeys` lets an older build drop it silently when it re-saves ([ADR-113](DECISIONS.md#adr-113)). The snapshot cannot show a changed default *value*; the corpus below does.
- **Document corpus** — `core/data/src/test/resources/fixtures/document-v{1,2,3}.json`: one fictional zine as each schema version wrote it (v3 from the serializer at `5f7707a`; v2/v1 are that output minus the fields they predate). `DocumentFixtureCorpusTest` decodes each file and compares it with a fully spelled-out expected document.
- **Library archive** — `core/data-storage/src/test/resources/fixtures/library-backup-v2.zine`: written at `5f7707a` (0.9.0-beta.5) by the unchanged `ZineLibraryBackupWriter` from those three documents and two synthetic JPEG photos; not a device export. `LibraryBackupFixtureTest` stages it through `ZineLibraryBackupStager` (the pure-JVM half of restore; the `:data-android` commit and Room reconcile are not exercised) and checks its projects, versions and assets.
- Each fixture's SHA-256 is recorded in its test, so a changed fixture byte fails the test. All content is fictional.
- **Privacy and dependencies** — `PrivacyManifestTest` (`:app`, Robolectric at SDK 28, plus one SDK 34 case) asserts the exact requested-permission set (no `INTERNET`) and `allowBackup=false`; `:app:checkDependencyAllowlist` fails CI when the resolved release graph contains a library group outside its listed groups, or inside its denied AndroidX subgroups.

**`.zine` v2 library backup, as written today** (contract: [ADR-110](DECISIONS.md#adr-110)): a ZIP whose first entry is `manifest.json` (`ZineLibraryBackupManifest`: `packageVersion` 2, `kind` `library`, `appVersion`, `createdAtEpochMs`, one entry per project and per asset), then `projects/<sourceProjectId>/document.json` — the project's document bytes exactly as stored, at whatever schema version (1…current) it was last saved — then one `assets/<sha256>` per distinct JPEG master. Every document and asset entry's byte count and SHA-256 is declared in the manifest and re-verified on restore, and entries are stored with no-compression DEFLATE. The size and count limits are the constants in `ZineLibraryBackupManifest.kt`; the JPEG-only MIME set and pixel-dimension limit are in `ZineLibraryBackupValidator.kt`, and the expansion-ratio gate in `ZineLibraryBackupStager.kt`. Restore refuses the whole backup if any project's `documentSchemaVersion` is newer than the build's. The single-project v1 package (`ZinePackageManifest`) has no production caller.

## 5. Rendering pipeline — one scene, two backends

WYSIWYG by construction ([ADR-006](DECISIONS.md#adr-006) principle; [ADR-027](DECISIONS.md#adr-027) concrete pure `:core:render` contract; [ADR-028](DECISIONS.md#adr-028) the Android replay/parity tier): a single pure function turns a `Page` into ordered, self-contained draw commands in page-local points; each backend supplies the points→target scale. Images emit *intent* and share one pure `computeImageBlit` (intrinsic from the backend's own decode); text emits *intent* and shares one Android `StaticLayout` path laid out in point space. **This guarantee is scoped to consumers of the draw-command tape** — every surface that renders a page. A transient editing affordance that paints document-owned pixels *outside* the replayer (today: the Reframe overlay, [ADR-053](DECISIONS.md#adr-053)) is not covered by it and owes its own parity proof, because sharing the pure geometry is only half the guarantee: it must also resolve that geometry from the same **inputs**. That second half is now structural too — every surface reads a master's intrinsic pixel size through the single pixel-free `readImageIntrinsics` seam ([ADR-056](DECISIONS.md#adr-056)), so the overlay and the renderer cannot resolve framing from different numbers (M7-01). If the shared intrinsic read fails, the feature layer cancels the tentative Reframe session without a document command and raises A25's visible support-paper snack; its polite live region speaks the same recovery line once, and the ordinary Add control returns as the truthful exit. The Android side ([ADR-028](DECISIONS.md#adr-028)) is a single `CanvasReplayer` invoked with two canvas providers — export PDF (drawing in **PostScript points**, with a *separate* image-decode pixel scale) and export raster (`×300/72` pixels); the editor-preview Compose host (`PagePreview`) shipped in S4 ([ADR-029](DECISIONS.md#adr-029)), replaying the same tape through the same replayer. Design + review trail: [spikes/core-render.md](spikes/core-render.md) (pure tier) + [spikes/core-render-android-backend.md](spikes/core-render-android-backend.md) (Android tier).
...
| S2A | `core:data` (pure core) | `core:model` | ✅ implemented — schema, serializer+migration, validation, repo/asset contracts, plus [ADR-110](DECISIONS.md#adr-110)'s v2 whole-library backup manifest and staged-archive validator ([spike §11](spikes/data-storage-layer.md#11-implementation-status--s2a-pure-kotlin-data-core-2026-06-19)) | S3 (no shared dep) |
| **S2B-core** | **`core:data-storage`** (pure JVM) | `core:data`, S2A | ✅ **on main** — atomic file source, autosave coordinator, content-addressed asset store, trusted streamed v2 backup reader/writer, private staging, and recoverable additive commit ([ADR-025](DECISIONS.md#adr-025)/[ADR-110](DECISIONS.md#adr-110)). **Mark-and-sweep GC deferred — not yet implemented** ([ADR-031](DECISIONS.md#adr-031) §2) | S3 (no shared dep) |
| S2B-android | `data-android` (Android library) | `core:data-storage` | ✅ **on main** — file-backed documents, autosave coordination, rebuildable Room project index ([ADR-042](DECISIONS.md#adr-042)), library backup/restore repositories, and thin SAF transport ([ADR-110](DECISIONS.md#adr-110)). WorkManager asset GC remains deferred. | S3 |
...
1. **Navigation graph expansion → ✅ closed for the project layer ([ADR-046](DECISIONS.md#adr-046)).**
   ...
   **Home/My-zines** pipeline landed across S6:
   ...
   thumbnails (S6.4, [ADR-045](DECISIONS.md#adr-045); **deleted unshipped**, [ADR-069](DECISIONS.md#adr-069)), and the **S6.5 re-root**
