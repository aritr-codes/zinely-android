...
**Backup implementation boundary (2026-08-21):** `:core:data` owns [ADR-110](DECISIONS.md#adr-110)'s v2 whole-library manifest and structural validator, additive beside the v1 single-project contract. `:core:data-storage` owns the hostile-input boundary: a `ZipFile`-backed, streaming reader verifies actual byte counts and SHA-256 values, strictly decodes and validates documents, proves exact image-asset closure, and stages the complete archive in a private temporary tree before exposing a result. The same pure layer provides collision-safe local-id allocation and a journaled additive commit primitive for complete, adapter-prepared project directories; caught failures roll back all new project visibility, and an interrupted journal is recoverable before Room reconciliation. `:data-android` now integrates that core into `RoomProjectRepository`: one exclusive lease in the existing autosave registry excludes editor writers, recovery precedes every files-to-Room scan, manifest metadata becomes an atomic `meta.json`, complete authoritative project directories and deduplicated assets commit before Room is reconciled, and collision remaps are returned in a repository receipt. SAF, storage-space UX, export production, and the frozen user surface remain unimplemented; Room remains a rebuildable index rather than backup authority.
...
| **S2B-core** | **`core:data-storage`** (pure JVM) | `core:data`, S2A | ✅ **on main** — atomic file source + autosave coordinator + content-addressed asset store (java.nio; CI-tested) ([ADR-025](DECISIONS.md#adr-025)); [ADR-110](DECISIONS.md#adr-110)'s streamed v2 ZIP verification/private staging and recoverable additive-commit primitives are implemented on the feature branch and integrated by `:data-android`'s repository transaction, with SAF still pending. **Mark-and-sweep GC deferred — not yet implemented** ([ADR-031](DECISIONS.md#adr-031) §2) | S3 (no shared dep) |
