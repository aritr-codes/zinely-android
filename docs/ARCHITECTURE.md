...
**Backup implementation boundary (2026-08-21):** `:core:data` now owns [ADR-110](DECISIONS.md#adr-110)'s pure v2 whole-library manifest and staged-archive validator, additive beside the v1 single-project contract. Byte hashing, transactional staging into private temporary storage, live-path commit, SAF, and the frozen user surface remain unimplemented; Room remains a rebuildable index rather than backup authority.
