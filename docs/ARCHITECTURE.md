...
**Backup implementation boundary (2026-08-21):** `:core:data` owns [ADR-110](DECISIONS.md#adr-110)'s v2 whole-library manifest and structural validator, additive beside the v1 single-project contract. `:core:data-storage` now owns the hostile-input boundary: a `ZipFile`-backed, streaming reader verifies actual byte counts and SHA-256 values, strictly decodes and validates documents, proves exact image-asset closure, and stages the complete archive in a private temporary tree before exposing a result. The same pure layer provides collision-safe local-id allocation and a journaled additive commit primitive for complete, adapter-prepared project directories; caught failures roll back all new project visibility, and an interrupted journal is recoverable before Room reconciliation. Android project-sidecar preparation, repository-wide writer-lock integration, startup recovery wiring, SAF, storage-space UX, and the frozen user surface remain unimplemented; Room remains a rebuildable index rather than backup authority.
