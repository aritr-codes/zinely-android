...
The user arrives asking: **“How is Zinely made, and what small defaults can I choose?”**

- The Colophon belongs to the **Shelf**, never the Bench or Proof.
- The Shelf dock presents `Backups` and `Colophon` as two equal, quiet secondary actions below the primary
  `Make a zine` action.
- It is named **Colophon**, not Settings, About, Privacy, or Licences. This is the printer's note in the
  small-press world established by ADR-103.
...
1. `Colophon` opens a full Shelf-owned destination.
...
4. Back from a licence returns to the Colophon at the originating row; a second Back returns to the Shelf.
...
| Paper heading | `Default paper` |
| Choices | `A4` · `US Letter` |
| Paper explanation | `Used first when you start a zine. You can still choose another.` |
| Typeface heading | `Typefaces` |
| Typeface roles | `Our handmade voice` · `Our editorial face` · `Our working face` |
| Licence label | `SIL Open Font License` |
| Privacy heading | `How Zinely works` |
| **The one product-level privacy sentence** | **`Zinely works offline, and your zines stay on this device unless you choose to share or back them up.`** |
| Version heading/value | `Version` · the real build version, not a hardcoded design value |
...
- In dark theme, the Colophon uses the current dark room/chrome treatment; its cards remain differentiated
...
- The screen exposes one `paneTitle`: `Colophon`; each licence child exposes its family name as `paneTitle`.
...
- Paper is one single-select group with two radio controls exposing selected state and an accepted-change
  announcement.
- Typeface rows are buttons named `<family>, SIL Open Font License` and do not announce decorative arrows.
- Back controls are `Back to your shelf` and `Back to Colophon` respectively.
...
The Colophon sentence above is the sole product-level reassurance. The frozen Library empty state, Bench add
...
- Back stack and focus-return tests for Shelf → Colophon → licence.
