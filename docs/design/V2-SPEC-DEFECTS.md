...
| **D-071** | ⏳ **an owner ruling — [OD-31](../DECISIONS.md#adr-098-od31); BLOCKS Phase D package D2. A probable NINTH amendment to a frozen V2 surface.** Raised 2026-08-06 by [ADR-098](../DECISIONS.md#adr-098) | [OD-12](#d-035-ruling) ruled *the artifact does not dim, the room around it may*, and said explicitly that the Proof **is not amended here** — but **`v2-proof.html` dims its own page**, which [D-035](#d-035)'s own cost note records in passing (*"it closes when Phase D brings the Proof onto V2"*). Phase D draws that page. Two questions, both the owner's: does the light-island rule extend to the Proof's `.zpage`, and if it does, what happens to `.zcap` (`:133–134`) — `#fff` serif text on a **fixed dark gradient** drawn *inside* the document page, which an island would leave unreadable |
