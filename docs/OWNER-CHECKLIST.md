...
| ☐ | Android 7–9 Save PDF on an emulator or device | Still open 2026-09-24: the only phone is Android 16 and no API 24–28 image is installed (a download of ~1 GB, not made without your say-so). Ask, grant, save twice, deny, Share, forced failure; no partial file in Downloads |
| ☐ | Samsung Pass 1 — run, one item fails | **Run 2026-09-24** on SM-A176B / Android 16 — every item executed; the focus item **fails** (row below). Nothing was printed on paper (fold guide only), and Backups was opened, not exercised. Results in the [tester package](releases/0.9.0-beta.5.md#on-device---samsung-sm-a176b-android-16-api-36-build-a176bxxs7czh3-2026-09-24). Scope was: Update over r3 keeps library + backups; About → Licences & credits → licence with hardware Back and focus at each level; `uiautomator` dump; light mode; 1.8× text; Reframe with a 4096 px photo; flipped spread (Bench, Proof, Read, print + fold); transparent PNG import; Undo across pages; Save PDF; Share |
| ☐ | Samsung Pass 2 — *your* first-time reading | An agent first-time reading was done 2026-09-24 (observations in the tester package); yours is still owed. Scope: About opening; finding Licences & credits; the transparent PNG's white box on cream; overall sanity |
| ☐ | TalkBack listen | Structural check done 2026-09-24 (names, roles, focus position); speech and gestures could not be driven over `adb`, so the listen is still owed. Scope: About, Licences & credits, licence screen, Save PDF and Share |
| ☐ | **Rule on the TalkBack focus finding** | On device, TalkBack focus on About / Licences & credits / licence lands on Back, the first paragraph or the shelf title, not where [COLOPHON-FREEZE](design/COLOPHON-FREEZE.md#accessibility-semantics) says. Everything stays reachable. Screens were opened by tap, not TalkBack double-tap, so a listen may show it differently. Fix before release, or accept as a Known Limitation |
