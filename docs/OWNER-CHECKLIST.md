...
| ☐ | [D-103](design/V2-SPEC-DEFECTS.md#d-103) | **`NOT AVAILABLE YET` breaks mid-word at font scale 1.8** — `NOT` / `AVAILABL` / `E YET`. ⚠ **Re-categorised after review:** a parity fix exists that needs no ruling (the freeze declares no `word-break`), but applying it makes the word **clip** instead — so the one line owed is *does legibility or parity win at large scale?* | Pass 2, 2026-08-20 on SM-A176B. Also corrects a prediction in `BenchArtSheet.kt` — at 1.8 it does **not** clip — and three wrong explanations I gave for why |
| ☐ | [D-104](design/V2-SPEC-DEFECTS.md#d-104) | **`TAPE & FIXINGS` now contains no tape**, and `CUT PAPER` shows a window and a speech tag. Closing the tile-level over-promise ([D-086](design/V2-SPEC-DEFECTS.md#d-086-ruling)) moved it up to the heading. Author the marks ([ADR-107](DECISIONS.md#adr-107)), reword the two headings, or accept it as temporary | Pass 2, 2026-08-20 on SM-A176B. Not a regression — a heading that mentions tape is a weaker false promise than the picture of tape it replaced |
...
- [D-103](design/V2-SPEC-DEFECTS.md#d-103) — at font scale 1.8 the unavailable label breaks mid-word
  (`NOT` / `AVAILABL` / `E YET`). Legible, not clipped. One line owed: legibility or freeze-parity?
