...
| ☐ | **Re-record the three Art-sheet goldens *again* (2026-08-20)** — `bench_art_sheet_light` · `bench_art_sheet_dark` · `bench_art_sheet_with_recents_light` | The A7 tile change moves them a second time: twelve tiles go from stroke to fill and four lose their glyph. ⚠ **The `*_compare.png` was read before this row was written** and shows exactly that and nothing else — no layout shift, no heading moved, the tint/tilt cycle intact. Same `record-goldens.yml`, same pinned image |
