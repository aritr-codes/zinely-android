...
| ☐ | [D-103](design/V2-SPEC-DEFECTS.md#d-103) | **`NOT AVAILABLE YET` breaks mid-word at font scale 1.8** — `NOT` / `AVAILABL` / `E YET`. Shorten the string (it is also what TalkBack speaks), cap the scale, or accept it | Pass 2, 2026-08-20 on SM-A176B. ⚠ Also **corrects my own carried-forward note**, which said the label *can clip*: measured, it does not clip — nothing is lost, it is only ugly. I had not run the scale when I wrote that |
