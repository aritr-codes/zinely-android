...
| Font / Size / Ink look disabled while typing and should not be shown if unavailable | **Current behavior is intentional but misleading.** `BenchStyleRow` draws three inert chips; only `Done` acts. Prior accessibility work explains them to TalkBack, but the visual first reading remains “broken.” The stakeholder independently reproduced the same weakness as the earlier F-1 review. | **P1 HTML ruling before Compose change** |
