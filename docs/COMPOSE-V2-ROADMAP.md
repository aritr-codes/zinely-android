...
| **C4** | **The bar, the status chip, the snackbar** | `.bar`, `.icon-btn*`, `.add*`, `.status`, `.saved*`, `.snack*` | C2 | ▶ **unblocked by [OD-9](design/V2-SPEC-DEFECTS.md#d-031-ruling), 2026-08-01.** The Read hand-off reuses [ADR-086](DECISIONS.md#adr-086)'s existing Editor → Proof route, back reuses the existing navigation stack, and **redo is kept** — so `Undo · Add · Done` is not read as an instruction to remove it. No new navigation architecture |
...
#### Blocks a named later package — four live, three closed by OD-2
...
| **OD-11** | **C2**, its `.ctx*` rows only | [**D-034**](design/V2-SPEC-DEFECTS.md#d-034) — the frozen `.ctx` is a **verb** bar (Edit · Font · Size · Ink · Delete); the shipped `EditorContextBar` it would replace is the **transform** bar that exists as the **WCAG 2.5.7** single-pointer alternative to dragging ([ADR-029](DECISIONS.md#adr-029) §6, [ADR-053](DECISIONS.md#adr-053) §5). `Delete` is all they share, and per-element `customActions` reach assistive tech only, so they do not substitute | ⛔ **live**, raised 2026-08-01 by C2's pre-implementation check. Four options are tabled in the entry, plus a fifth from review: **split C2 into C2a (selection, proceeds now) and C2b (`.ctx`, fenced)** — the same row-not-package fencing OD-10 got at C1 |
