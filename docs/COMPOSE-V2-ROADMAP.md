...
| **D1** | **`:core:ui`'s eleven `Z*` components onto V2** — the widest golden event in the phase, paid once | none | — | ⏳ **BLOCKED on [OD-41](DECISIONS.md#adr-098-od41)** (raised 2026-08-07). ~~▶ **UNBLOCKED 2026-08-06** — its only blocker, OD-37, is answered by `fc33bca`, and it needs no design ruling.~~ OD-37's answer removed D1's **baseline** blocker and nothing else; D1 needs no *design* ruling but does need a **scope** ruling, and [ADR-097 §7](DECISIONS.md#adr-097) deviation 5 (`Accepted`) already answered it restrictively — *"outside a parity phase's fence."* **Not started; Phase D has not opened.** `ZButton` · `ZSheet` · `ZSnackbar` · `ZMenuItem` · `ZStatusPane` · `ZToast` · `ZPaperSurface` · `ZTextField` · `ZFocusRing` · `ZSweep` · `ZAccessibleControl` are **100% V1**, and both the V2 Bench and the V2 Proof consume them. Re-records `v2_catalog_*` (10) and ~100 `feature/editor` rasters |
...
everything else waits on a design ruling that does not yet exist. *Recording that a package is unblocked is not
the same as opening it; Phase D has not begun.*
