...
| **C4** | **The bar, the status chip, the snackbar** | `.bar`, `.icon-btn*`, `.add*`, `.status`, `.saved*`, `.snack*` | C2 ✅ | ⛔ **BLOCKED 2026-08-04 — [ADR-094](DECISIONS.md#adr-094) opened `Proposed`, property table populated, no production code written.** Unblocked by [OD-9](design/V2-SPEC-DEFECTS.md#d-031-ruling) on 2026-08-01 (Read reuses [ADR-086](DECISIONS.md#adr-086)'s Editor → Proof route, back reuses the existing stack, **redo is kept**, no new navigation architecture) — but C4's own **[pre-implementation blocker check](DECISIONS.md#adr-094-blockers)** then found two questions OD-9 does not reach. ⛔ **[D-047](design/V2-SPEC-DEFECTS.md#d-047)**: the frozen `.bar` draws **three** slots (`Undo · Add · Done`) while OD-9 keeps redo, [OD-11](design/V2-SPEC-DEFECTS.md#d-034-ruling)/[OD-14](design/V2-SPEC-DEFECTS.md#d-039-ruling) keep **both** shipped add verbs, and the frozen `Add`'s destination (`openSupply()` → the `.sheet`) is **C8**, which OD-2 re-seated beyond Phase C. **Three** readings are left open — they are materially different products, one needs a **frozen-Bench amendment** and one needs a slice of C8's fence released early. Geometry is not the obstacle. ⚠ Corrected at C4's implementation gate ([ADR-094 §5](DECISIONS.md#adr-094-gate)): the frozen `Add` opens a chooser that *does* exist in the freeze — `openSupply()` `:694-705`, narrating *"Add stays three verbs — Text · Photo · Art"* — two of whose verbs ship today. ⛔ **[D-048](design/V2-SPEC-DEFECTS.md#d-048)**: binding the bar's `Done` to the Read hand-off duplicates the shipped top-end `Preview ›` (`EditorScreen.kt:625-637`), which OD-14 forbids — yet `doneBtn`'s two *frozen* jobs are already owned by C3's `.done` chip and OD-13's tap-outside, so **every reading collides with OD-14**. Blocks row 4.8 only. 🟦 **[D-049](design/V2-SPEC-DEFECTS.md#d-049)** is carried, not blocking: [C2a's Pass 2](DECISIONS.md#adr-091-completion-device) measured the sheet shrinking **17 %** on every select and dismiss (`1028×1454` → `850×1202`) because the bottom chrome takes its space from the canvas, and routed the decision here. **Buildable today, against rulings already in hand:** `.status`, `.saved`, `.snack`, the soft-delete and its undo ([ADR-094 rows 4.9–4.15](DECISIONS.md#adr-094-fpt)) — the `✿` U+273F question is pre-answered by [D-021](design/V2-SPEC-DEFECTS.md#d-021-ruling). Every C4 address ADR-089 gave is **stale** after four amendments and is re-anchored in [ADR-094 §1](DECISIONS.md#adr-094-drift) |
...
| **OD-19** ⏳ | **C4** | [**D-047**](design/V2-SPEC-DEFECTS.md#d-047) — the frozen `.bar` (`v2-bench.html:441-444`) draws **three** slots, `Undo · Add · Done`. [OD-9](design/V2-SPEC-DEFECTS.md#d-031-ruling) keeps **redo**; [OD-11](design/V2-SPEC-DEFECTS.md#d-034-ruling)/[OD-14](design/V2-SPEC-DEFECTS.md#d-039-ruling) keep **both** shipped add verbs (`Add photo`, `Add words`); and the frozen `Add`'s own destination, `openSupply()` → the `.sheet`, is **C8** — re-seated beyond Phase C by OD-2, so there is nothing shipped for it to reuse. The bar absorbing everything needs a **frozen-Bench amendment**; the bar staying additive beside `EditorSupplyTray` **violates OD-14**, drawing Undo and Add twice at once; the bar as drawn with `Add` opening the freeze's own chooser (`openSupply()` `:694-705`, *"Add stays three verbs — Text · Photo · Art"*) minus its `Art` row builds **C8's fence** early and leaves redo without a slot. Geometry is not the obstacle — four controls fit. **What the bar may contain is the question** | ⛔ **BLOCKS C4.** Raised 2026-08-04 by C4's [pre-implementation blocker check](DECISIONS.md#adr-094-blockers) |
| **OD-20** ⏳ | **C4 row 4.8** | [**D-048**](design/V2-SPEC-DEFECTS.md#d-048) — ADR-089 row 4.6 and OD-9 send the Read hand-off to the bar's `Done`, but it already ships as the top-end `Preview ›` (`EditorScreen.kt:625-637`); binding both presents one action twice, which **OD-14 forbids**. Transcribing `doneBtn` literally instead is no better: its two frozen jobs (`v2-bench.html:629`) are `endEdit()` — built by C3 as the `.done` chip — and `deselect()` — built by C2a under [OD-13](design/V2-SPEC-DEFECTS.md#d-037-ruling). **Every reading collides with OD-14**, so what `Done` *means* on this screen is the owner's | ⛔ **BLOCKS C4 row 4.8 only.** The rest of C4 is unaffected |
| **OD-21** ⏳ | **C4** | [**D-049**](design/V2-SPEC-DEFECTS.md#d-049) — the sheet measures `1028×1454 px` unselected and `850×1202 px` selected, a **17 % linear shrink**, because the bottom chrome takes its space from the canvas (`.bar{flex:none}`, `v2-bench.html:244`). Pre-existing, but OD-13's dismissal means the user now meets it on **every** deselect, in the growing direction — and [OD-12](design/V2-SPEC-DEFECTS.md#d-035-ruling) rules that the editor represents the physical printed artifact. Take the height as frozen, or overlay the chrome and amend | 🟦 **Does not block.** Raised as **P2-1** by [C2a's Pass 2](DECISIONS.md#adr-091-completion-device), which measured it and routed it here |
