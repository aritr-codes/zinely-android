...
| # | Package | Frozen regions | Depends on | Status |
|---|---|---|---|---|
| **D0** | **Enrolment definition + corpus repairs** — documentation and test-infrastructure only, zero `src/main` | none (`@media`/token blocks read, not transcribed) | — | ▶ **UNBLOCKED 2026-08-09 by the [OD-29](DECISIONS.md#adr-098-od29) ruling** — disposition (b): discipline is **traceability to the frozen V2 source**, and enforcement is D0's to build under that rule but not to redefine. ~~⏳ **BLOCKED on OD-29**~~ (OD-37 answered 2026-08-06). D0 now **records** what token discipline means for a V2 surface rather than deciding it, makes `config/token-enrolment.txt` a declared Gradle input, and re-anchors the three stale addresses on Phase D's own path. **It does not enrol anything** |
...
| ~~**OD-29**~~ | ~~D0~~ | ~~What does token discipline *mean* for a V2 surface, given D-007 published no scale? Until it is defined, **no V2 package can enrol at all**~~ | ✅ **ANSWERED 2026-08-09 — disposition (b): traceability.** Discipline means **every value traces to the frozen V2 source**, not that no literal appears; where the V2 design deliberately keeps a value at component or caller level, keeping it there **is** compliance. The literal ban could not survive [D-006](design/V2-SPEC-DEFECTS.md#d-006--the-only-shape-token-in-v2-is-declared-and-never-used) and [D-007](design/V2-SPEC-DEFECTS.md#d-007--the-constitutional-8pt-rhythm-is-not-observable-in-the-frozen-css), which left no destination for the values it banned — `com.aritr.zinely.ui.components` alone holds **126** `.dp`, **27** `.sp`, **12** `RoundedCornerShape(`. **Enforcement delegated to D0**, which may narrow the mechanism but not the rule. Ruling and its corpus repair: [OD-29](DECISIONS.md#adr-098-od29) |
| ~~**OD-42**~~ | ~~D1~~ | ~~[D-076](design/V2-SPEC-DEFECTS.md#d-076) sub-case A — 18 rows whose V1 token has no §7.1 job: assign the V2 job from the use, or from the nearest role by colour?~~ | ✅ **ANSWERED 2026-08-09 — (a) + (y).** Assign from **what the element does at the use site**; **one-to-one not required**; no V2 role invented. [OD-42](DECISIONS.md#adr-098-od42) |
| ~~**OD-43**~~ | ~~D1~~ | ~~[D-076](design/V2-SPEC-DEFECTS.md#d-076) sub-case B — 7 rows whose V1 chrome hue collides with *no fourth chrome hue* and *cover inks never in the interface*~~ | ✅ **ANSWERED 2026-08-09 — (a): the use changes.** The job survives, **the hue does not**; V2-TOKENS **not** amended. Priced: a visible hue change on three V1-era controls. [OD-43](DECISIONS.md#adr-098-od43) |
| ~~**OD-44**~~ | ~~D1~~ | ~~[D-076](design/V2-SPEC-DEFECTS.md#d-076) sub-case C — does V1's second paper step survive into V2?~~ | ✅ **ANSWERED 2026-08-09 — (a): it does not.** The frozen trilogy declares `--paper`/`--paper-edge` and nothing between; V1's step is a **title-hash** cover device. **No token added.** [OD-44](DECISIONS.md#adr-098-od44) |
| ~~**OD-45**~~ | ~~D1~~ | ~~[D-077](design/V2-SPEC-DEFECTS.md#d-077) shadow — does ADR-071's Bench-canonical rule reach a use-site-authored shadow?~~ | ✅ **ANSWERED 2026-08-09 — (b1): no; the caller supplies it.** `--frame-shadow` is a **colour**; no `:root` declares a geometry, so there was nothing canonical to inherit. [ADR-074](DECISIONS.md#adr-074) unamended. [OD-45](DECISIONS.md#adr-098-od45) |
| ~~**OD-46**~~ | ~~D1~~ | ~~[D-077](design/V2-SPEC-DEFECTS.md#d-077) duration / easing — same question for values ADR-075 says are not tokens~~ | ✅ **ANSWERED 2026-08-09 — ruled separately.** **Duration:** a use-site frozen value, per caller — ADR-071's rule is conditioned on *"where Library omits"*, and the Library declares its own `.24s`. **Easing:** already settled; `--settle`/`--standard` are declared tokens and stay shared. [OD-46](DECISIONS.md#adr-098-od46) |
...
**Updated 2026-08-07, after the OD-41 ruling.** ~~**D0 waits on OD-29.**~~ *(struck 2026-08-09 — OD-29 answered; see the 2026-08-09 note below.)* **D1 is ▶ unblocked** — OD-41 answered,
...
**Updated 2026-08-09, after the six-question owner-decision session.** **D0 is ▶ unblocked** —
[OD-29](DECISIONS.md#adr-098-od29) is answered, disposition (b), so *"until this is defined no V2 package can
enrol at all"* no longer holds; **D9's enrolment hold dissolves** and D9 narrows to the deletion exactly as its
row already anticipated. **D1 is unchanged at ▶ unblocked**, and gains what it lacked: with
[OD-42](DECISIONS.md#adr-098-od42)…[OD-46](DECISIONS.md#adr-098-od46) answered, its package ADR **can now reach
`Accepted`** rather than only `Proposed`. **Both packages with no dependency are therefore open**, which is the
first time in the phase that has been true. Everything from D2 onward still waits on a design ruling that does
not yet exist — **ten of the eighteen ODs remain owed** — and no package boundary, dependency or numbering
changed. D0 now owes one repair it did not before: `config/token-enrolment.txt`'s coupling clause still names
**V1's** migration targets, against [ADR-080](DECISIONS.md#adr-080) Decision 2.
