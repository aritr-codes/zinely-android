...
| **C2b** | **The contextual toolbar** — `.ctx` and its verb sets | `.ctx*` (rows 2.10–2.13) | **C2a's acceptance** | ▶ **unfenced by [OD-13](design/V2-SPEC-DEFECTS.md#d-037-ruling)** — the ruling C2b was waiting on is made. It begins when C2a is accepted, i.e. when both re-run device passes succeed. Carries [OD-11](design/V2-SPEC-DEFECTS.md#d-034-ruling)'s non-removal invariant: the frozen verb bar is **additive** and `EditorContextBar` stays reachable |
