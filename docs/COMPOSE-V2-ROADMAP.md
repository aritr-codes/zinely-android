...
>
> ***Dated snapshot — superseded. As of 2026-08-06, C0 through C6 are all DONE and ACCEPTED and the next
> package is [C9](#phase-c-packages). The paragraph below is kept as the record of 2026-08-01, for the same
> reason the B5 paragraph above it is kept: a plan edited to match its outcome stops being evidence.***
>
...
| **C9** | **Integration** — the four states, the motion policy, persistence of place, both device passes, the phase gate | `@media (prefers-reduced-motion)` and the state machine across all of the above | C1–C6 ✅ | ▶ **READY — the next package, and nothing fences it. Its planning package is [ADR-097](DECISIONS.md#adr-097), 📝 `Proposed` 2026-08-06, opened before any production code with a 14-row property table; it awaits owner GO.** Dependencies met: C1, C2a, C2b, C3, C4, C5 and C6 are all `Accepted` (C6 on 2026-08-06). **This row read ⛔ blocked until 2026-08-06 and the label was wrong** — it named [D-012](design/V2-SPEC-DEFECTS.md#d-012--the-three-frozen-files-write-three-different-reduced-motion-rules-and-one-of-them-would-strobe) as a blocker while its own next clause, [the register's own ledger](design/V2-SPEC-DEFECTS.md) and [HANDOVER §0](COMPOSE-V2-HANDOVER.md) all say the same entry fences nothing. D-012 is **carried, not blocking**: its behavioural decision was deferred *to this phase, on physical devices*, so it is answered **in** C9 as row 9.2 rather than before it. Corrected here; no new decision is made by the correction. Persistence of place is the **page half only**; the shelf half travels with H1. Also carries C1's one open device finding: `StickerBlob` announces `✿`/`❀`/`★` to the platform a11y tree against [EditorEmptyState]'s documented contract — pre-existing, found by [C1's Pass 1](DECISIONS.md#adr-090-device-verification) |
