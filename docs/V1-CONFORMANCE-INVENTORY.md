...
- **Required** [§2.1](ZINELY-DESIGN-SYSTEM.md) five registers — **frozen, and no sixth** ([ADR-067](DECISIONS.md#adr-067)); [§6](ZINELY-DESIGN-SYSTEM.md) ~~eleven roles (sixteen if CI-10 accepts A-6)~~ **sixteen roles, every one carrying a named register** ([ADR-067](DECISIONS.md#adr-067), [ADR-068](DECISIONS.md#adr-068)); [§13](ZINELY-DESIGN-SYSTEM.md) *"No sizes invented for this screen."*
- **Authority** as CI-20
- **Milestone** C0 · **Prereq** ~~CI-10~~ **satisfied** ([ADR-067](DECISIONS.md#adr-067), [ADR-068](DECISIONS.md#adr-068) — **sixteen** roles, **every one with its register named**) — **UNBLOCKED** · **Kind** documentation · **Changes** docs
...
#### CI-99 · Six type roles have no register, and Heading is genuinely ambiguous — ✅ **DONE**
- **Status** ✅ **CLOSED 2026-07-24 — [ADR-068](DECISIONS.md#adr-068)**, owner ruling **Option (b)**: **Heading → Display**, Button/Label/Instruction/Warning → **Body**, Empty state → **Display**. **All sixteen §6 roles now carry a register.** §6's Display reservation is **⚠️ AMENDED** from three uses to four, admitting *"the title of a primary product surface, where that title forms part of the product's editorial identity"* — the never-for-a-label prohibition retained verbatim. **Option (c) — register from context — explicitly rejected: a role's register is part of the system and is never selected during implementation.** Five-register architecture unchanged; §2.1 untouched. **The four serif sites stay conformant, so no surface is restyled and no golden moves.**
...
- **Required** [§6](ZINELY-DESIGN-SYSTEM.md) ~~eleven roles by purpose (sixteen if CI-10 accepts A-6)~~ **sixteen roles by purpose** ([ADR-067](DECISIONS.md#adr-067)), **all sixteen mapped to a §2.1 register** ([ADR-067](DECISIONS.md#adr-067) five, [ADR-068](DECISIONS.md#adr-068) six, five whose name is their register) — **this item maps roles onto registers; it does not negotiate them.** What remains is reducing the *sizes*: Display alone ships at 19, 21, 22 and 26sp across four sites, and one register takes one size; [§2.1](ZINELY-DESIGN-SYSTEM.md) five registers, **frozen**; [§13](ZINELY-DESIGN-SYSTEM.md) *"No sizes invented for this screen."*
...
- **Verify** If wired: the shelf draws the user's page 1, verified on device with a real zine. If deleted: `HomeModule` no longer constructs the renderer, no PNG is written on edit, and the shelf goldens are re-recorded once.
